...
from CNN.model.utils import NECKS, Conv_Module
...
class DepthwiseConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), dilation=(1, 1), freeze_bn=False):
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), dilation=1, freeze_bn=False):
        super(ConvBlock, self).__init__()
        self.conv = nn.Conv2d(in_channel, out_channels, kernel_size, stride=stride, padding=padding)
        self.bn = nn.BatchNorm2d(out_channels, momentum=0.9997, eps=4e-5)
        self.act = nn.ReLU()
...
class BiFPN(nn.Module):
    def __init__(self, in_channels, out_channel=64, num_layers=2, epsilon=0.0001):
        super(BiFPN, self).__init__()
        self.p3 = nn.Conv2d(in_channels[0], out_channel, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))
        self.p4 = nn.Conv2d(in_channels[1], out_channel, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))
        self.p5 = nn.Conv2d(in_channels[2], out_channel, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0))

        # p6 is obtained via a 3x3 stride-2 conv on C5
        self.p6 = nn.Conv2d(in_channels[2], out_channel, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))

        # p7 is computed by applying ReLU followed by a 3x3 stride-2 conv on p6
        self.p7 = ConvBlock(out_channel, out_channel, kernel_size=3, stride=2, padding=1)

        bifpns = []
        for _ in range(num_layers):
            bifpns.append(BiFPNBlock(out_channel))
